...
from lib.data_access_layer.crud import (add_entities, add_entity,
                                        ...
                                        remove_all_entities, remove_entity,
                                        update_entity)
...
@pytest.fixture
def mock_model():
    with patch('your_flask_app.db.Model', new_callable=lambda: SampleModel) as mock:
        yield mock

@pytest.fixture
def mock_session(mock_model):
    with patch('your_flask_app.db.session', new_callable=lambda: SampleModel()) as mock:
        yield mock
...
def test_remove_all_entities(mock_model, mock_session):
    remove_all_entities(mock_model)
